...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&20130603&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&77
  ...
  if (NULL == *con_cls)
   ...
   //	if(MHD_set_connection_value(connection,MHD_HEADER_KIND,MHD_HTTP_HEADER_CONTENT_TYPE, MHD_HTTP_POST_ENCODING_FORM_URLENCODED)==MHD_YES)
//				printf("enctype= url  successs!\n");
   	if(MHD_set_connection_value(connection,MHD_HEADER_KIND,MHD_HTTP_HEADER_CONTENT_TYPE, MHD_HTTP_POST_ENCODING_MULTIPART_FORMDATA)==MHD_YES)
		...
		printf(" now  lookup  type    %s  \n " ,MHD_lookup_connection_value(connection,MHD_HEADER_KIND,"type") );
		printf(" now  lookup  filenamE    %s  \n " ,MHD_lookup_connection_value(connection,MHD_HEADER_KIND,"filename") );
		printf(" 2       now  lookup  filenamE    %s  \n " ,MHD_lookup_connection_value(connection,MHD_POSTDATA_KIND,"filename") );
  ...
  if (0 == strcmp (method, "PUT"))
	...
	printf("before  MHD_post_process  upload_data_size is %d \n",*upload_data_size);
      if (0 != *upload_data_size)
		...
		printf("now is  MHD_post_process  ,after  \n");
          *upload_data_size = 0;
  ...
  if (NULL == *con_cls)
	...
	//	printf("cheek  is  %s \n",cheek);
      ...
      if (0 == strcmp (method, "POST"))
		...
		printf(" post:MHD_create_post_processor sucess!\n");


		printf("  after  create;     now  lookup  filenamE    %s  \n " ,MHD_lookup_connection_value(connection,MHD_POSTDATA_KIND,"filename") );
		printf("  after  create;     now  lookup  filenamE    %s  \n " ,MHD_lookup_connection_value(connection,MHD_HEADER_KIND,"filename") );
          nr_of_uploading_clients++;
